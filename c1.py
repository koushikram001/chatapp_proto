...
def receive_message_from_server(sck, m):
    while True:
        ...
        #test
        # print(from_server)
        # chat_his = open("c1.txt" , "a")
        # print(from_server)
        # chat_his.write(from_server + ";")
        # chat_his.close()
        #test
...
def getChatMessage(msg):
    ...
    tkDisplay.config(state=tk.NORMAL)
    first_three_client_chr = msg[:3]
    if first_three_client_chr == "@ai" :
        tkDisplay.insert(tk.END, "\nYou->" + "called the Ai_bot","tag_your_message")
        #print(reply)
...
def send_mssage_to_server(msg):
    ...
    text_dis=tkDisplay.get("1.0", tk.END).strip()
    print(text_dis)
    f = open("c1.txt","w")
    f.write(text_dis)
    f.close()
